...
def sample_test_entity(test_triplets, ratio):
    all_test_entities = set()
    for (head, tail, _) in test_triplets:
        all_test_entities.update([head, tail])
    n_removed_entities = int(ratio*len(all_test_entities))
    removed_entities = set(np.random.choice(list(all_test_entities), 
                                            size=n_removed_entities, 
                                            replace=False))
    return set(removed_entities)


def remove_triplet_from_train(train_triplets, removed_entities):
    new_train_triplets = []
    for (head, tail, relation) in train_triplets:
        if head in removed_entities or tail in removed_entities:
            pass
        else:
            new_train_triplets.append((head, tail, relation))
    return new_train_triplets


def remove_edges_from_test_partially_inductive(test_triplets, removed_entities, inductive_edges):
    new_test_triplets = []
    test_dict = dict()
    for (head, tail, relation) in test_triplets:
        if head in test_dict:
            test_dict[head].append((tail, relation))
        else:
            test_dict[head] = [(tail, relation)]
    for head in test_dict.keys():
        if head in removed_entities:
            n_sample_triplet = min(inductive_edges, len(test_dict[head]))
            sampled_triplets_idx = list(np.random.choice(range(len(test_dict[head])), 
                                                         size=n_sample_triplet, 
                                                         replace=False))
            new_test_triplets.extend([(head, test_dict[head][idx][0], test_dict[head][idx][1]) for idx in sampled_triplets_idx])
        else:
            new_test_triplets.extend([(head, tail, relation) for (tail, relation) in test_dict[head]])
    random.shuffle(new_test_triplets)
    return new_test_triplets
...
def load_data(model_args):
    ...
    if args.inductive:
        removed_entities = sample_test_entity(test_triplets, args.ratio)
        train_triplets = remove_triplet_from_train(train_triplets, removed_entities)

        if args.partially_inductive:
            test_triplets = remove_edges_from_test_partially_inductive(test_triplets, removed_entities, args.inductive_edges)
