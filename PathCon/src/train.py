...
def train(model_args, data):
    ...
    with tf.Session() as sess:
        ...
        if args.inductive:
            if not args.partially_inductive:
                with open('inductive_log.txt', 'a+') as f:
                    f.write(f"{args.ratio},{max(all_hit1)}\n")
            else:
                with open('partially_inductive_log.txt', 'a+') as f:
                    f.write(f"{args.ratio},{args.inductive_edges},{max(all_hit1)}\n")
